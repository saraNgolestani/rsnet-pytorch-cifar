...
class ResNet(ptl.LightningModule):
    ...
    def forward(self, x):
        ...
        out = self.layer1(out)
        print(f'layer 1 shape:{out.size}')
        out = self.layer2(out)
        print(f'layer 2 shape:{out.size}')
        out = self.layer3(out)
        print(f'layer 3 shape:{out.size}')
        out = self.layer4(out)
        print(f'layer 4 shape:{out.size}')
        out = F.avg_pool2d(out, 4)
        print(f'avg shape:{out.size}')
        out = out.view(out.size(0), -1)
        print(f'view shape:{out.size}')
        out = self.linear(out)
        return out
